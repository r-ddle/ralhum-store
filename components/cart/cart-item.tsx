...
export function CartItem({ item }: CartItemProps) {
  ...
  const isLowStock = item.variant.inventory <= 5;
  const isOutOfStock = item.variant.inventory === 0;
  const maxQuantity = Math.min(10, item.variant.inventory); // Limit to 10 or available inventory
    ...
    <div className="flex gap-4 p-4 bg-white rounded-lg border border-gray-200 hover:shadow-sm transition-shadow">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mb-2">
          ...
          <p className="text-xs text-[#003DA5] font-medium mt-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center border border-gray-300 rounded-md">
              ...
              <Button
                ...
                onClick={() => handleQuantityChange(item.quantity + 1)}
                disabled={
                  isUpdating || item.quantity >= maxQuantity || isOutOfStock
                }
              >
